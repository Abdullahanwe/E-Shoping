...
import { SignatureLevelEnum } from "../utils/security/token.security";
import { TokenDocument, TokenRepository, UserDocument, UserModel, UserRepository } from "src/DB";
...
export class TokenService {
    ...
    detectSignatureLevel = async (role: RoleEnum): Promise<SignatureLevelEnum> => {
    ...
    getSignature = async (signatureLevel: SignatureLevelEnum = SignatureLevelEnum.Bearer): Promise<{
        ...
        return {
            ...
            refresh_signature: signatures.refresh_signature
        }
    }
    ...
    createRevokeToken = async (decoded: JwtPayload): Promise<TokenDocument> => {
        ...
        if (result) {
            throw new BadRequestException('Fail to generate revoke token')
        }
        return result;
    }
}